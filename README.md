...
- **日次利用制限**: リクエスト回数・入力/出力トークン数の日次上限設定と超過時の自動制限
...
アプリケーションは以下の日次上限によるAPI使用量制限を実装しています：

- **リクエスト回数制限**: 日次リクエスト上限（デフォルト100回）
- **入力トークン制限**: 日次入力トークン上限（デフォルト5,000,000トークン）
- **出力トークン制限**: 日次出力トークン上限（デフォルト100,000トークン）

利用制限に達した場合、以下のエラーメッセージが表示され、新規リクエストはブロックされます。制限は日本時間の午前0時にリセットされます。設定値は環境変数で調整可能です。
...
**「本日のリクエスト回数制限を超過しました」**
- `DAILY_REQUEST_LIMIT`を増加させるか、翌日を待つ必要があります。

**「本日の入力トークン制限を超過しました」**
- `DAILY_INPUT_TOKEN_LIMIT`を増加させるか、翌日を待つ必要があります。
- 
**「本日の出力トークン制限を超過しました」**
