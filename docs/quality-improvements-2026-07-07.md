...
- データをXMLタグで区切って送信: `<カルテ情報>` `<紹介目的>` `<現在の処方>` `<追加情報>`
...
- pytest: 522件すべてパス（新規テスト9件追加: JSON判定、再生成プロンプト、stop_reason警告、system prompt受け渡し、スペース保持）
- pyright: 今回の変更由来のエラーなし
- npm run build: 成功

## 今後の検討事項（未実装）

- 評価結果のJSON構造化出力（ハルシネーション指摘リスト・軸別スコア）とフロントエンドでの構造化表示
- セクション分割を正規表現パースからtool useによる構造化出力へ移行
- 評価スコアのDB保存による品質推移の計測
